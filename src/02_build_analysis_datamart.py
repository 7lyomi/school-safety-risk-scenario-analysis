...
def parse_grade_group(row):
    ...
    if school == "초등학교":
        if "2학년" in grade or "3학년" in grade:
            return "초등학교_중학년(2-3)"
        if any(value in grade for value in ["4학년", "5학년", "6학년"]):
            return "초등학교_고학년(4-6)"
        return "초등학교_기타"
...
def main() -> None:
    ...
    mart_df = total_df[
        total_df["학교급"].isin(["초등학교", "중학교", "고등학교"])
    ].copy()

    semester_contexts = mart_df["사고연월"].apply(parse_semester_context)
    mart_df["파생_계절"] = [value[0] for value in semester_contexts]
    mart_df["파생_학기맥락"] = [value[1] for value in semester_contexts]
