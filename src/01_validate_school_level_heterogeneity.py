...
def main() -> None:
    ...
    with pd.ExcelWriter(OUTPUT_FILE) as writer:
        time_pivot = total_df.pivot_table(index="사고시간", columns="학교급", aggfunc="size", fill_value=0)
        time_pivot.to_excel(writer, sheet_name="사고시간_맥락_비교")
        type_pivot = total_df.pivot_table(index="사고형태", columns="학교급", aggfunc="size", fill_value=0)
